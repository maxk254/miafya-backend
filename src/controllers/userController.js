import validator from "validator"; // to validate the users email from the data base
import bcrypt from "bcrypt"; // to hash the pasword for security
...
const registerUser = async (req, res) => {
  try {
    ...
    if (password.length < 8) {
      return res.json({
        success: false,
        message: "You password must contain atleast 8 characters",
      });
    }
...
const loginUser = async (req, res) => {
  try {
    ...
    // to compare the entered loging user detsils are simmilar to the usermodel data in the database
